               ...
               Transition::MoveRelativeStateConsume(j, pos, c) => {
                   ...
                   match rest {
                       Ok(o) => {
                           let mut fsm = o;
                           match c {
                               ColorType::Output(_) => {fsm.outp.push((new_point, color, c))},
                               ColorType::Input(_) => {fsm.inpt.push((new_point, color, c))},
...
pub struct FSMResult {
    ...
    inpt: Vec<(Point, Color, ColorType)>,
    outp: Vec<(Point, Color, ColorType)>,
    capt: Vec<(u8, i32)>,
}
...
pub struct Identified{
    glyph: Glyph,
    pub identifier: GlyphType,
    inputs: Vec<(Color, ColorType)>, // program color input, function input
    outputs: Vec<(Color, ColorType)>, // Program color input, function input
    // flowin: i32
    // flowout: i32
...
impl DefinitionLedger {
    ...
    pub fn identify(&self, glyph: &Glyph, pic: &Picture) -> Option<Identified> {
        ...
        let mut id: GlyphType = GlyphType::CustomFunction(0);
        ...
        let mut inputs = vec![];
        let mut outputs = vec![];
        for (_, c, ct) in ok_fsm.inpt {
            if inputs.contains(&(c, ct)) {
                inputs.push((c, ct))
            }
        }
        for (_, c, ct) in ok_fsm.outp {
            if outputs.contains(&(c, ct)) {
                outputs.push((c, ct))
            }
        }
    ...
    pub fn load_dir(&mut self) {
        ...
        let ledger = ColorLedger{
            inputs: vec![],
            outputs: vec![],
            function: Color {r:255, g:255, b:0}
        };
        let glyphs = gather_glyphs(&pic, ledger);
